...
export const Asset: React.FC<IMyDibsPageParams> = observer(({ lot }) => {
  ...
  return (
    <VStack padding="2rem" gap="2rem">
      ...
      <HeaderTitleChip icon={icon} name={name} assetResearch={assetResearch} />
      <HStack w="100%" gap="2.75rem">
        {headerFields.map(({ param, value }) => (
      ...
      <Tabs>
        ...
        <TabPanels>
          <TabPanel>
            <SimpleGrid
              columns={columnsCount === 4 ? 1 : 2}
              gridTemplateColumns={columnsCount === 4 ? null : '1fr 3fr'}
              spacing="0.75rem"
            >
              {columnsCount === 3 && (
              ...
              <VStack gap="1.5rem" w="full">
                <HStack w="100%">
                  <motion.div layout>
                    ...
                    >
                      <Button w="100%" onClick={toggleColumnsCount}>
                        Filters
                      </Button>
                      <InputGroup>
                        <InputLeftElement>
