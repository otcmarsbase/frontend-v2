...
import {EmptyData, List, LotRow, LotStatus, LotTypeChip, UserDataChip} from '@shared/ui-kit';
                ...
                itemRender={(item) => (
                    <LotRow
                        lot={{
                            ...
                            isHot: item.isHot,
                            handleClickLot: ({ lotId }) => console.log(lotId),
                            status: <LotStatus value={item.status} />,
                            fields: [
                                {label: 'Lot Type', value: <LotTypeChip headingProps={{variant:'h6'}} lotType={item.lotType}/>},
                                {label: 'Published at', value: format(item.publishedAt, 'dd.MM.yyyy')},
                                {label: 'Bid FDV', value:
                                        <HStack fontWeight={600}>
                                            <Text whiteSpace='nowrap'>
                                                {item.fdv.toLocaleString('en-US', {maximumFractionDigits: 0})}
                                            </Text>
                                ...
                                {label: 'Offer Maker', value: <UserDataChip offerMaker={item.offerMaker} offerMakerIcon={item.offerMakerIcon} />},
                                {label: 'Direct Seller/Or not', value: item.isDirectSeller ? 'yes' : 'nope'},
                                {label: 'Location', value: <Heading fontSize='1rem' fontWeight='500' lineHeight='1.5rem'>{item.location}</Heading>},
                            ],
                        }}
                    />
                )}
