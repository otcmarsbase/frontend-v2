...
export const UserCard = ({ id, nickname, isVerifyKYC, lastLoginDeviceType, lastLoginTime }: UserCardProps) => {
  ...
  const formattedLastLoginTime = useMemo(
    () => lastLoginTime && format(lastLoginTime, 'dd.MM.yyyy HH:mm'),
    [lastLoginTime],
  );
    ...
    >
      ...
      <HStack spacing="1rem">
        <VStack alignItems="flex-start">
          <Text fontSize={fontSize} color="dark.50">
            ID
          </Text>
        ...
        <VStack alignItems="flex-start">
          ...
          <HStack alignItems="center">
            <Text fontSize={fontSize}>
              {lastLoginDeviceType} | {formattedLastLoginTime}
            </Text>
          </HStack>
        </VStack>
      </HStack>
