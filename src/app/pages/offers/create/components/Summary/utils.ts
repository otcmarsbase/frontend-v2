...
export const getTargetFields = ({step, lotType, pricingModel}: IGetTargetFieldsProps): Array<string[]> => {
    ...
    if (EStepTypes.FIRST_STEP === step) {
        ...
        targetFields = Object.entries(StepsText[step][lotType][isPricingModelExist ? pricingModel : EPricingModel.IN_STABLECOIN])
    }
    return targetFields
}

export const normalizeFields = ({targetFields, data}): INormFieldsReturn[] =>{
