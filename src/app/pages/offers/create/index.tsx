...
export const CreateOffer: React.FC = observer(() => {
    ...
    function handleRecountValues({currentID, bindedID, value, pricingModel}: IHandleRecountValue) {
        ...
        //@ts-ignore
        const denomByLotType_ = form.getValues(StepThreeRecountFieldByLotType[data.lotType]);

        console.log('handleRecountValues', contractValue_, roundFDV_, denomByLotType_, StepThreeRecountFieldByLotType[data.lotType])
        if (!contractValue_ || !roundFDV_ || !denomByLotType_) {
            // @ts-ignore
            form.setValue(currentID, value);
            return
        }

        const denomByPricingModel = new Decimal(form.getValues(pricingModel === EPricingModel.IN_STABLECOIN ? 'contractSizeToOffer' : 'equityToOffer'))
        ...
        // @ts-ignore
        const denomByLotType = new Decimal(denomByLotType_);
...
console.log('_bindedID',_bindedID,_currentID)
        // @ts-ignore
