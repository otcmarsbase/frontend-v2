...
export function RoundInfoFields() {
  ...
  const { formState, control, watch, setValue, trigger } = useFormContext<RoundInfoModel>();
  ...
  const handleTotalEquityBought = useCallback(
    (value: number) => {
      setValue('totalEquityBought', value);
      trigger('totalEquityBought');

      if (contractValue) {
        setValue('pricePerEquity', value ? contractValue / value : undefined);
        trigger('pricePerEquity');
      }
    },
    [contractValue, setValue, trigger],
  );

  const handlePricePerEquity = useCallback(
    (value: number) => {
      setValue('pricePerEquity', value);
      trigger('pricePerEquity');

      if (contractValue) {
        setValue('totalEquityBought', value ? contractValue / value : undefined);
        trigger('totalEquityBought');
      }
    },
    [contractValue, setValue, trigger],
              ...
              <Controller
                ...
                render={(props) => (
                  <InputGroup>
                    <UIKit.InputNumber
                      {...props.field}
                      onChange={handleTotalEquityBought}
