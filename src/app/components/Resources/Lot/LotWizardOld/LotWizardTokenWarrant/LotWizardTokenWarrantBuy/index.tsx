...
import { StartInfoFields, ProjectInfoFields, LotInfoFields } from './_steps';
import { ReviewStep } from './_steps/REVIEW';
import { CreateStepDictionary, SafeBuyStepType } from './const';

export function LotWizardTokenWarrantBuy() {
  const [step, setStep] = useState<SafeBuyStepType>('START_INFO');
      ...
      <Box p="2rem" position="relative" display="flex" flexDirection="column" justifyContent="space-between">
        <Box position="sticky" top="2rem">
          <Steps
            ...
            canClickItem={(item) => !isSubmitting && stepDictionary.backSteps.indexOf(item) !== -1}
        ...
        <VStack position="sticky" bottom="2rem" mt="10rem" gap="1rem">
          {step === 'REVIEW' ? (
            <Button w="full" color="white" type="submit" isLoading={isSubmitting}>
              Publish Lot
            </Button>
          ) : (
