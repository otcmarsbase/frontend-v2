...
import { AssetName, DealType, HotChip, LotTypeChip, NumberText } from '@shared/ui-kit';
...
export const LotCard: React.FC<LotCardProps> = ({ lot, onClick }) => {
  ...
  const fields: FieldType[] = useMemo(() => {
    ...
    return [
      ...
      lot.deadline
        ? {
            name: 'Lot Deadline',
            value: <Text>{formatDate(new Date(lot.deadline), 'dd/MM/yyyy')}</Text>,
          }
        : null,
      {
        name: 'Vertical',
  ...
  return (
    ...
    >
      <Box flexShrink="0">
        <DealType invert value={lot.direction} position="absolute" top="0" right="0" />
