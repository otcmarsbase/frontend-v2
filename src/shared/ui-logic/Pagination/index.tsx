...
export const Pagination: React.FC<PaginationProps> = ({
  ...
  const [currentPageFromItem, currentPageToItem] = useMemo(() => {
    ...
    return [
      fromItem >= totalItems ? totalItems : fromItem,
      toItem >= totalItems ? totalItems : toItem,
    ];
...
function prepareButtons(
  ...
  const beforeSplitter: PaginationViewButtonProps =
    currentPage - centerPageGroupNumber >= 0 && currentPage !== 1
      ? {
          key: 'before_splitter',
          page:
            currentPage - centerPageGroupNumber > 1
              ? currentPage - centerPageGroupNumber
              : 1,
          display: '...',
          isSelected: false,
        }
  ...
  const afterSplitter: PaginationViewButtonProps =
    currentPage + centerPageGroupNumber - 1 <= lastPage &&
    currentPage !== lastPage
      ? {
          key: 'after_splitter',
          page:
            currentPage + centerPageGroupNumber < lastPage
              ? currentPage + centerPageGroupNumber
              : lastPage,
          display: '...',
          isSelected: false,
  ...
  const [minPage, maxPage] = [
    currentPage - leftRightElements,
    currentPage + leftRightElements,
  ];

  const centerPages: PaginationViewButtonProps[] = new Array(
    maxPage - minPage + 1,
  )
    .fill(null)
