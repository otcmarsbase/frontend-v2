...
export interface NullableObject
  ...
  isEmptyUseNullable<T>(
    value: T | null | undefined | NullableObject,
  ): T | NullableObject;
...
export const NullableObject = (<T>(
  ...
  if (NullableObject.isEmpty(value))
    return NullableObject.isEmpty(defaultValue) ? NullableObject : defaultValue;
  return value;
}) as NullableObject;
...
NullableObject.isExtends = (value: any): value is NullableObject => {
  return (
    ...
    (typeof value === 'object' &&
      !!value &&
      value[NULLABLE_TYPE] === NULLABLE_TYPE)
  );
};
NullableObject.isEmpty = (value: any): value is NullableObject => {
  return (
    NullableObject.isUndefined(value) ||
    NullableObject.isNull(value) ||
    NullableObject.isExtends(value)
  );
};
NullableObject.hasValue = <T>(
  value: T | NullableObject | null | undefined,
): value is T => {
  return !NullableObject.isEmpty(value);
};
NullableObject.isEquals = (
  other: NullableObject | null | undefined,
): boolean => {
...
NullableObject.isEmptyUseNullable = <T>(
  value: T | null | undefined | NullableObject,
) => {
...
NullableObject.resolveDeep = (<T>(value2: T): NullableResolveDeep<T> => {
  const deeper = (_value: any) => {
    if (NullableObject.isNull(_value))
      return undefined as NullableResolveDeep<T>;
