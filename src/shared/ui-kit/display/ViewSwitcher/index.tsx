...
export function ViewSwitcher({ stackProps, initialValue, onChange }: ViewSwitcherProps) {
  ...
  return (
    <HStack {...stackProps}>
      <Button width="2.25rem" height="2.25rem" bg="rgba(37, 38, 40, 0.5)" onClick={() => onClick('GRID')}>
        <UIIcons.Common.GridIcon
          width="1.5rem"
          height="1.5rem"
          color={selectedViewType === 'GRID' ? 'orange.300' : 'currentColor'}
        />
      </Button>
      <Button width="2.25rem" height="2.25rem" bg="rgba(37, 38, 40, 0.5)" onClick={() => onClick('LIST')}>
        <UIIcons.Common.ListIcon
          width="1.5rem"
          height="1.5rem"
          color={selectedViewType === 'LIST' ? 'orange.300' : 'currentColor'}
        />
      </Button>
    </HStack>
  );
