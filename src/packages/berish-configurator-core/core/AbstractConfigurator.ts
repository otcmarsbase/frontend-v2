...
export abstract class AbstractConfigurator<Schema extends Record<string, any>> implements IConfigurator<Schema> {
  ...
  get<Key extends keyof Schema & string>(key: Key, parser?: (value: string) => Schema[Key]): string | Schema[Key] {
    const rawValue = this.raw(key);
    return this.use(rawValue, (data) =>
      String(data) === '' ? null : typeof parser === 'function' ? parser(data) : data,
    );
  }

  defaults<T>(value: T | null | undefined, defaultValueFactory: () => T): T {
    return typeof value === 'undefined' || value === null ? defaultValueFactory() : value;
  }

  use<T, K>(value: T | null | undefined, initCallback: (value: T) => K): K {
    return typeof value === 'undefined' || value === null ? (value as undefined) : initCallback(value);
  }
