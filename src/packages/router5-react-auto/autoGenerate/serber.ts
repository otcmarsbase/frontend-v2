...
import { Serber, plugins, ISerberPlugin, SERBER_PATH_SYMBOL } from '@berish/serber';
...
const routePlugin: ISerberPlugin<React.ComponentType, React.ComponentType, ISerberParams> = {
  isForSerialize(obj) {
  ...
  beforeSerialize(obj, params) {
    params[SYMBOL_SERBER_ROUTER5_AUTO_MAP] = params[SYMBOL_SERBER_ROUTER5_AUTO_MAP] || [];
  },
  serialize(obj, params) {
    let paths = LINQ.from(params[SERBER_PATH_SYMBOL]).ofType(['string', 'number']) as LINQ<string | number>;

    if (params[SYMBOL_SERBER_HOME_PAGE_ITEM]) {
      const homePageItem = params[SYMBOL_SERBER_HOME_PAGE_ITEM].toString().toLocaleLowerCase();

      paths = paths.filter((m) => m.toString().toLocaleLowerCase() !== homePageItem);
    }

    if (params[SYMBOL_SERBER_PARAM_PAGE_ITEM]) {
      paths = paths.map((key) => paramPageItemConvert(key, params[SYMBOL_SERBER_PARAM_PAGE_ITEM]));
    }

    params[SYMBOL_SERBER_ROUTER5_AUTO_MAP].push([paths.map((m) => m.toString()).toArray(), obj]);
...
export function paramPageItemConvert(key: string | number, paramPageItem: string) {
  if (paramPageItem && key && typeof key === 'string') {
    if (key.startsWith(paramPageItem) && key.endsWith(paramPageItem)) {
      ...
      const paramName = pageSplittedItems.slice(1, pageSplittedItems.length - 1).join(paramPageItem);
