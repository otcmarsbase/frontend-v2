...
export const PriceInfo: FC<{
    ...
    const {form, helperText, label, lotType, handleRecountPriceInfoValues} = props;
    const {register, getValues, formState} = form;
    const {errors} = formState;
    const curIds = STEP_TWO_PRICE_INFO_LABELS_BY_LOT_TYPE[lotType].map(item=>item.id)
    return (
        <HStack>
                        ...
                        >
                            <FormControl>
                                ...
                                <Input placeholder={'Amount'}
                                       ...
                                       type={'number'}
                                       id={item.id}
                                       onChange={(e) => handleRecountPriceInfoValues(curIds, e.currentTarget.id, e.currentTarget.value)}
                                />
                            </FormControl>
                        </HStack>
                    })}
