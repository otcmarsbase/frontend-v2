...
> = T extends `${infer Prefix}/${Key}${infer Suffix}`
	? `${Prefix}/${Value}${Suffix}`
	: T extends `${infer Prefix}/${Key}`
	? `${Prefix}/${Value}`
	: T extends `/${Key}${infer Suffix}`
	? `${Value}${Suffix}`
	: T extends `/${Key}`
	? `${Value}`
	: never
...
export type RemapBranch<Branch, Prefix extends string> = {
	[K in keyof Branch]: Branch extends { _: infer Prefix2 extends string }
		? Branch[K] extends string
			? K extends '_'
				? { short: Branch[K]; full: Join<[Prefix, Branch[K]]> }
...
export const ROUTES = {
	dashboard_offerDetails: '/dashboard/offer-details/:id',
	//   dashboard_bidDetails: '/dashboard/bid-details/:id',
	...
	createBidForOffer: '/offer/:id/create-bid',
	createBidForOfferNew: '/offer-new/:id/create-bid',
	offerDetails: '/dashboard/offer/:id/',
	otcDesk: '/otc-desk',
	createOffer: '/create-offer',
	createOfferNew: '/create-offer-new',
